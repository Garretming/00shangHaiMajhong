...
----------------------------------------------------------------
--客户端发送亮牌操作丢弃的牌
P.CLI_LIANG_CARD                 =0x2012
CLIENT[P.CLI_LIANG_CARD]={
    ver=1,
    fmt={
...
SERVER[P.SVR_ENDDING_BROADCAST] = {
    ...
    fmt = {
        ...
        {name = "birdCount",type = T.INT},--捉的鸟数
        ...
        {name = "players", type = T.ARRAY,fixedLength = 4,
            fmt = {
                    ...
                    fmt = {
                        ...
                        {name="pinghu",type = T.ARRAY,request="hutype",requestValue=1,fixedLength = 1,
                            fmt = {
                                ...
                                {name="ifgangpao",type = T.BYTE},   --是否杠上炮
                ...
                {name="isOnline",type = T.SHORT},   --是否在线
                {name="coins",type = T.INT},   --用户总钱数
                {name="changeCoins",type = T.INT},    --本轮输赢

                {name="remainCardsCount",type = T.INT},   --用户剩余手牌
                ...
                -- {name="pgCount",type = T.INT},--补杆个数
                {name="bgCount",type = T.INT},--补杆个数
                {name="dgCount",type = T.INT},--点杆个数
                ...
                --花牌
                {name="hCount",type = T.INT},
                {name="hCards",type=T.ARRAY,fixedLengthParser = "hCount",fixedLength = 0,
                    fmt = {
                        {type = T.BYTE},   --pai
                     }
                },
                ...
                --是否平胡
                {name="isPinghu",type = T.BYTE},

                --是否碰碰胡
                {name="isPengpenghu",type = T.BYTE},

                --是否七对
                {name="isQidui",type = T.BYTE},

                --是否十三烂
                {name="isShisanlan",type = T.BYTE},

                --是否单吊
                {name="isDandiao",type = T.BYTE},

                --是否清一色
                {name="isQingyise",type = T.BYTE},

                --是否混一色
                {name="isHunyise",type = T.BYTE},

                --是否字一色
                {name="isZiyise",type = T.BYTE},

                --是否天胡
                {name="isTianhu",type = T.BYTE},

                --是否地胡
                {name="isDihu",type = T.BYTE},

                -- --是否门清
                -- {name="isMenQing",type = T.BYTE},

                -- --是否门清无花果
                -- {name="isMQWuHuaGuo",type = T.BYTE},

                --是否四财神
                {name="isSiaishen",type = T.BYTE},

                --是否财吊
                {name="isCaidiao",type = T.BYTE},

                --飘财数量
                {name="piaoCaiCount",type = T.BYTE},
...
SERVER[P.SVR_PLAYER_USER_BROADCAST] = {
    ...
    fmt = {
        ...
        {name = "card", type = T.BYTE}, --牌
        {name = "lid", type = T.INT}, --如果是碰，杠，胡，杠胡。ID为上一个出牌用户的座位ID
        ...
        {name = "tingCards", type = T.ARRAY,fixedLengthParser = "tingCount",fixedLength = 0,
            fmt = {
                ...
                {name = "tingHuCards", type = T.ARRAY,fixedLengthParser = "tingHuCount",fixedLength = 0,
                    fmt = {
                        {name = "huCardsCount", type = T.BYTE},--听胡的牌数量
                        {name = "huCards", type = T.ARRAY,fixedLengthParser = "huCardsCount",fixedLength = 0,
                            fmt = {
                                {name = "card", type = T.BYTE},--能听胡的牌
                            }
                        },
                        {name = "componentCount", type = T.BYTE},--这张牌能组成的顺子
    ...
    fmt = {
        ...
        {name = "tingCards", type = T.ARRAY,fixedLengthParser = "tingCount",fixedLength = 0,
            fmt = {
                ...
                {name = "tingHuCards", type = T.ARRAY,fixedLengthParser = "tingHuCount",fixedLength = 0,
                    fmt = {
                        {name = "huCardsCount", type = T.BYTE},--能听胡的牌数
                        {name = "huCards", type = T.ARRAY,fixedLengthParser = "huCardsCount",fixedLength = 0,
                            fmt = {
                                {name = "card", type = T.BYTE},--能听胡的牌
                            }
                        },
                        {name = "componentCount", type = T.BYTE},
                        {name = "component", type = T.ARRAY,fixedLengthParser = "componentCount",fixedLength = 0,
                            fmt = {
                                {name = "card", type = T.BYTE},--这张牌能组成的顺子
                            }
                        }
                    }
                }
...
SERVER[P.SVR_HUPAI_BROADCAST] = {
    ...
    fmt = {
        ...
        {name = "hucontent", type = T.ARRAY,fixedLengthParser = "hu_count",fixedLength = 0,
            fmt = {
                {name="uid",type = T.INT},   --uid
                {name="hutype",type = T.SHORT,cache=1},   --胡牌类型
                ...
                {name="pao_content",type = T.ARRAY,request="hutype",requestValue=1,fixedLength=1,
                    fmt = {
                        {name="paoid",type = T.INT},   --点炮id
                ...
                {name="remainCardsCount",type = T.INT},   --剩余的手牌
                {name = "remainCards", type = T.ARRAY,fixedLengthParser = "remainCardsCount",fixedLength = 0,
                    fmt = {
                        {type = T.BYTE},   --牌
                    }
...
SERVER[P.SVR_REGET_ROOM] = {
    ver = 1,
    fmt = {
        {name = "handleCard", type = T.BYTE}, --当前出牌
        {name = "handle", type = T.INT}, --自己可以的操作
        {name = "currentPlayerId", type = T.SHORT}, --重连出牌的用户座位ID
        {name = "nTableType", type = T.SHORT}, --
        {name = "seat_id", type = T.SHORT}, --重连用户座位ID
        {name = "gold", type = T.INT}, --重连用户拥有金币数 
        {name = "nTingFlag", type = T.SHORT}, --
        {name = "qiangTouCard", type = T.SHORT},
        {name = "caiShenCount", type = T.BYTE}, --财神牌张数
        {name = "caiShenCards", type = T.ARRAY,fixedLengthParser = "caiShenCount",fixedLength = 0,
            fmt = {
                {type = T.BYTE},   --牌
            }
        },
        {name = "isbaibantishen", type = T.BYTE}, --是否为白板替身
...
------------------------------------------------------------------------------------------------------
                    ...
                    {name = "pCards", type = T.ARRAY,fixedLengthParser = "pCount",fixedLength = 0,
                        fmt = {
                                {name = "pCards", type = T.BYTE},--其他用户信息: 用户所碰的牌
                            }
                    },                       
                    ...
                    {name = "gCards", type = T.ARRAY,fixedLengthParser = "gCount",fixedLength = 0,
                        fmt = {
                                ...
                                {name = "isAg", type = T.BYTE},--其他用户信息: 杠牌的类型
                        }
                    },

                    --花牌
                    {name = "hCount", type = T.SHORT},--其他用户信息:花牌的数量
                    {name = "hCards", type = T.ARRAY,fixedLengthParser = "hCount",fixedLength = 0,
                        fmt = {
                            {name = "card", type = T.BYTE},--其他用户信息:花牌的数量
                        }
                    },
...
--------------------------------------------------------------------------------------------------------
        ...
        --花牌
        {name = "hCount", type = T.SHORT},--重连用户信息:花牌的数量
        {name = "hCards", type = T.ARRAY,fixedLengthParser = "hCount",fixedLength = 0,
            fmt = {
                {name = "card", type = T.BYTE},--重连用户信息:花牌的数量
            }
        },
...
----------------------------------------------------------------------------------------------------
        ...
        {name = "tingCount", type = T.BYTE},--听牌数组大小
        {name = "tingCards", type = T.ARRAY,fixedLengthParser = "tingCount",fixedLength = 0,
            fmt = {
                {name = "card", type = T.BYTE},--当前听的牌
                {name = "tingHuCount", type = T.BYTE},
                {name = "tingHuCards", type = T.ARRAY,fixedLengthParser = "tingHuCount",fixedLength = 0,
                    fmt = {
                        {name = "huCardsCount", type = T.BYTE},--能听胡的牌数
...
SERVER[P.SVR_SEND_USER_CARD] = {
    ...
    fmt = {
        ...
        {name = "qiangTouCard", type = T.BYTE},--翻出的墙头牌
        {name = "caiShenCount", type = T.BYTE}, --财神牌张数
        {name = "caiShenCards", type = T.ARRAY,fixedLengthParser = "caiShenCount",fixedLength = 0,
            fmt = {
                {type = T.BYTE},   --牌
...
SERVER[P.SVR_LIANGDAO] = {
    ...
    fmt={
        ...
        {name="cardSum",type=T.BYTE},    --亮倒操作可选的组合数
        {name="LiangDate", type=T.ARRAY, fixedLengthParser="cardSum", fixedLength=0,
            fmt={
                {name="OpCard",type=T.BYTE},    --丢出去的一张牌
                {name="huCardsSum", type=T.BYTE},    --剩余手牌的听牌数据
                {name="huCards", type=T.ARRAY, fixedLengthParser="huCardsSum", fixedLength=1,
                    fmt = {
                        {name = "card", type = T.BYTE},   --牌
                    }
                },
                {name="componentSum", type=T.BYTE},  --最后不成形的组合数据
                {name="component", type=T.ARRAY, fixedLengthParser="componentSum", fixedLength=1,
                    fmt = {
                        {name = "card", type = T.BYTE},   --牌
                    }
                }
...
SERVER[P.SVR_GROUP_TIME] =  {
    ...
    fmt = {
        ...
        {name = "round", type = T.INT},--0，总轮次
        {name = "round_total",type =T.INT }--当前轮次
    }
}
...
--补花
P.SERVER_BUHUA = 0x4022
SERVER[P.SERVER_BUHUA] = {
    ver = 1,
    fmt = {
        {name = "seatId", type = T.BYTE}, --位置ID
        {name = "huaSize", type = T.BYTE}, --花牌数量
        {name = "huaList", type = T.ARRAY, fixedLengthParser = "huaSize", fixedLength = 0,
            fmt = {
                {name = "card", type = T.BYTE}, --手牌中补花的牌
            }
        },
        {name = "cardsSize", type = T.BYTE},
        {name = "cardsList", type = T.ARRAY, fixedLengthParser = "cardsSize", fixedLength = 0,
            fmt = {
                {name = "card", type = T.BYTE}, --补手牌的花之后摸起来的牌
            }
        }
    }
}
