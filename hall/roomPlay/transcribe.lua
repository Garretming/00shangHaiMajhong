        ...
        cct.httpReq2({
            ...
            callBack=function(data)
                if data.returnCode=="0" then
                    ...
                    print("self.game_type------",self.game_type)
...
function transcribe:repleaseToNewRoom(datatb)
    -- body
    local video_ex =  SCENENOW["scene"]:getChildByName("video_ex")
    if video_ex ~= nil then
        SCENENOW["scene"]:removeChildByName("video_ex")
    end
    local vidio_ex = require("hall.roomPlay.transcribeScene_ex")

    SCENENOW["scene"]:addChild(vidio_ex.new(datatb,self.game_type))
...
function transcribe:parsing(datatb)
    ...
    if self.game_type == "湘阴麻将" 
    or self.game_type == "赣州冲关"
    or self.game_type == "宜宾麻将" then
        isRun=false
        self:repleaseToNewRoom(datatb)
        return
    end
    ...
    print(self.game_type,"game_name")
